...
- **Statut (2026-08-06)** : ✅ `build_progress_payload(progress, deck_id, cards_count, *, last_card_position=…)` (`backend/content/views.py:884`) est le seul endroit qui calcule `effective`/`progress_pct` et le rattrapage de `cards_seen_count`, et les 4 vues l'appellent. Deux points ont demandé plus que l'extraction brute. (1) **Le rattrapage `seen` coûte une requête** : `DeckListCreateView` le résout déjà en lot pour tous les decks affichés, donc le paramètre `last_card_position` permet de passer la position déjà connue (`None` = pas de rattrapage) ; les autres appelants l'omettent et la fonction fait le lookup elle-même via `_last_card_position`. Un test de constance du nombre de requêtes (1 deck vs 2) verrouille l'absence de N+1 sur la liste. (2) **`start/` et `progress/` ne rattrapaient pas `seen`** : ils renvoyaient `cards_seen_count` brut, donc un deck repris affichait 0 % sur ces réponses et 75 % sur la liste — c'est précisément l'incohérence visée, les 4 endpoints renvoient maintenant la même valeur. Le `deck_id` propre à ces deux réponses reste ajouté par l'appelant (`{"deck_id": …, **payload}`), les clés du payload sont inchangées. 4 tests dans `backend/content/tests.py` (`OfficialDeckProgressPayloadTests`) : progression absente, égalité des compteurs/pct sur les 4 endpoints, rattrapage depuis `last_card_id` partout, constance des requêtes.
...
- **Statut (2026-08-06)** : ✅ `content/serializers.py` devient un package (`serializers/outputs.py` pour les réponses, `serializers/inputs.py` pour les 20 serializers d'entrée, un par endpoint qui accepte un corps), sur le modèle de l'éclatement de `views.py` en §3.1 ; les vues se réduisent à `serializer.is_valid(raise_exception=True)` puis à la lecture de `validated_data` (−314 lignes sur les 5 modules de vues concernés, `thumbs.py` passe de 175 à 99 lignes). Quatre points ont demandé plus que la traduction mécanique. (1) **Le format des erreurs n'était pas encore homogène après coup** : `DRFValidationError({"champ": "message"})` produit une *chaîne*, un serializer produit une *liste*, et `ListField` indexe en plus les erreurs de ses éléments (`{"champ": {0: [...]}}`). Les rares 400 qui restent dans les vues (contrôles qui ont besoin d'objets déjà chargés, comme « Some ids are not in this pack ») ont été passés en liste, et `FlatListField` aplatit les erreurs d'éléments : toute l'app renvoie désormais `{"champ": ["message"]}`. (2) **La validation manuelle laissait filer des valeurs jusqu'à la base** : `name` n'était borné à aucune longueur alors que `Deck.name` et `Subject.name` sont des `CharField(60)`/`(120)`, et `cover_image_id` n'était pas vérifié — un id inconnu remontait en `IntegrityError`, donc 500. Les serializers reprennent les bornes des modèles et vérifient l'existence de l'image : 400 dans les deux cas. (3) **Des entrées invalides étaient silencieusement ignorées** : un `mode_last` hors énumération et un `sort_order` non entier (sur `SubjectCardDetailView`) donnaient un 200 sans avoir rien écrit ; ils sont maintenant refusés. (4) **`estimated_minutes`/`cover_image_id` n'acceptaient `""` qu'en PATCH**, pas en POST — `NullableIntegerField` traite `""` comme `null` des deux côtés. Trois résolutions passent par des champs dédiés plutôt que par une recherche dans la vue : `MicroArticleSlugField` (en `filter(...).first()`, pas `get()` : un slug Wagtail n'est unique que sous un même parent), `HexColorField` et `WagtailImageUploadField` (qui rejoue la validation image de §1.5). Laissé volontairement en l'état : `difficulty` reste du texte libre alors que le modèle porte des `choices`, parce que le back-office l'expose comme un champ texte ; et les paramètres de *query string* (`AdminMicroArticleSearchView`, filtres du feed) gardent leur analyse manuelle, tolérante par construction. 24 tests dans `backend/content/tests.py` (`InputSerializerValidationTests`), dont ceux qui verrouillent les trois changements de comportement ci-dessus.
...
- **Statut (2026-08-06)** : ✅ `MicroArticleCardSerializer` (`backend/content/serializers/outputs.py`) est désormais la source unique du noyau d'une fiche (textes riches assainis, points clés, couverture/crédits, tags et date de publication) et de ses champs éditoriaux optionnels (taxonomies, questions, flux, points récap). Les vues `content`, le SRS de `learning` et l'ancienne API `product` l'utilisent directement ; `_microarticle_list_item`, les helpers de couverture/tags/catégories/questions et leurs copies dans `learning`/`product` ont été supprimés. Les méthodes `api_*` ont également quitté le modèle : `MicroArticleCardField` adapte les champs du serializer à Wagtail v2 en conservant les clés historiques (`api_key_points`, `api_cover`, etc.). L'adaptateur accepte `raw_data` (Wagtail 7) avec repli `stream_data` pour les versions antérieures. Deux régressions verrouillent la parité du payload entre `content`/`product`/Wagtail v2 et `learning`; les tests existants de constance des requêtes sur les listes restent verts. Aucun changement de schéma ni migration.
...
- **Statut (2026-08-06)** : ✅ `drf-spectacular` expose le contrat v1 sur `GET /api/schema/` et le schéma validé est suivi dans `openapi/schema.yaml`. Les `APIView` et serializers décrivent désormais les requêtes/réponses des domaines auth, contenu, decks/packs, sujets, taxonomies et apprentissage ; la génération stricte passe avec zéro avertissement, y compris sans base initialisée. `frontend/src/lib/types.ts` est entièrement généré par `openapi-typescript` (`npm run types:generate` / `types:check`) et le workflow `.github/workflows/openapi-contract.yml` régénère le schéma puis échoue si le schéma ou les types suivis ont dérivé. Deux drifts réels ont été trouvés grâce au typecheck : le lecteur consommait `is_read` sans que le détail API le renvoie (champ désormais fourni), et utilisait encore `categories_classes` au lieu de `categories_theme` (alias obsolète supprimé côté frontend). 107 tests backend, génération/validation OpenAPI, check des types, TypeScript et ESLint passent.
